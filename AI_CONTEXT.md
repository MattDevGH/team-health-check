...
**Where the work is:** ask git. `git status`, `git log --oneline -10` and
`gh pr list` answer it correctly; a sentence here cannot. This paragraph used to
read "Integration hardening is on `feat/integration-hardening`" — merged
2026-08-26 — alongside the claim that `requirement-coverage` "has never
executed" when it runs on every pull request, and that "Task 26 is the only work
remaining" across eleven specs written after it. Every session is told to read
this file first, so the first thing every session learned was wrong.

**Preserve the persistent `prisma/dev.db`.** It carries the development team and
sessions that the browser acceptance passes were driven through, and recreating
them costs an afternoon.

The block below is generated by `scripts/generate-status.ts` and checked in CI.
It carries only what changes when a spec changes — no branch, no commit, no
date, no test count — because anything else would go stale on unrelated work and
the check would be switched off.
...
**733 of 740 task boxes are ticked**, across 12 specs.

Specs with open boxes:

| Spec | Open |
|---|---|
| `deployment` | 3 of 62 |
| `reaching-your-health-check` | 1 of 51 |
| `slack-sign-in` | 3 of 77 |
...
**What this checkpoint was.** On 2026-08-23 a team was driven through two
complete health-check sessions in a browser, and the closure audit that followed
found completion claims nothing supported. Both are recorded below and in the
milestone sections further down.

*This paragraph used to end "Resume with Task 26 (reconciliation and merge)" and
list the tasks then in flight. Task 26 merged 2026-08-26, and eleven specs have
been written since. A resume cue is a fact about today written in a file nobody
updates on the day; the generated block above is where today lives now.*
...
"Uses immediate ack pattern: respond 200 within 3 seconds" sat above code that
resolved the member, looked up the open session, upserted a response per button,
fetched a question title for each, and POSTed an outbound reply — all before
returning the 200 Slack waits three seconds for. It failed under ordinary Turso
latency rather than under attack, and the outbound reply had no timeout at all
until the day before.

**Written down, acknowledged, then done.** The buttons go into the interaction
queue that already existed for failed deliveries, the route answers, and the
drain runs immediately afterwards. Not `void process(...)`: a serverless runtime
may stop unawaited work once the response is flushed, which would trade a
visible failure for a silently dropped answer. If this instance stops between
the two, a later scheduler tick finds the entry.

Applying had to leave the route to make that possible — `applyScoreActions`
takes its dependencies, and `createProductionScoreActionDeps` builds the single
set both callers use, which is what makes a replayed entry behave like a live
one.

Two decisions worth keeping:

- **Applying is the delivery; the reply is a courtesy on top.** The entry counts
  as delivered once the scores are stored, even if the member could not be told.
  Retrying would re-apply answers they already gave in order to send a
  confirmation they may no longer be waiting for — and the upsert is idempotent
  while a reply is not.
- **The acknowledgement is unconditional.** A failure in the drain is logged and
  left in the queue rather than reaching the response, because Slack retries a
  failed interaction by replaying the whole thing.

Proved by holding the post-response work instead of running it, which is the
only way to see the order from outside: the route answers 200 with nothing yet
stored, the entry is in the queue, and running the held work stores it. Move the
work back in front of the acknowledgement and exactly those two tests go red
while the other 21 pass.
