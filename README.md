...
1. **Auth/session and audit contracts:** remaining team-route auth and ownership
   checks, direct AuthContext documentation, micro-pulse selection,
   reused-session close scoping, and the still-missing schedule-change and
   member-addition audit emissions.
...
```bash
npm test            # unit + property tests (985 tests across 120 Vitest files)
