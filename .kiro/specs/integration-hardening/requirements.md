...
1. WHEN a valid Session_Link token is accessed, THE `/api/auth/session-link/[token]` endpoint SHALL return a response containing: memberId, sessionId, memberName, cadencePreference, the list of Questions (id, title, description, displayOrder), and any existing Responses (field name: `responses`) for that member in that session.
2. WHEN the member has a cadencePreference of "micro_pulse", THE response SHALL include only the selected question(s) per the weighted random algorithm, with an expandable flag indicating remaining questions are available.
